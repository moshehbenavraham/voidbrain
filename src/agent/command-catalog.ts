...
export const AGENT_COMMAND_CATALOG: readonly AgentCommand[] = [
	{
		...
		name: "Ingest source",
		intent: "Convert a synthetic source record into vault-ready artifacts without mutating user notes directly.",
		status: "planned",
		privacyLevel: "local-first",
		writePolicy: "staged-changes",
		prerequisites: [
			"Source path is vault-relative and validated.",
			"Provider calls are disabled unless a later explicit review allows them.",
			"Generated notes link back to source records.",
		],
		inputs: [
			{
				name: "sourcePath",
				description: "Vault-relative path to a synthetic source note or source record.",
				required: true,
			},
		],
		outputs: [
			{
				...
				description: "Reviewable generated note paths and support-record paths.",
				required: true,
			},
		],
		requiredEvidence: ["source path", "generated note paths"],
		...
		recoveryBehavior: "Leave source path and staged artifact IDs in the command log for retry or discard.",
		notes: ["Full ingestion execution is planned for a later session."],
	},
	{
